...
dirty candidate pass, and one clean implementation pass for sparse Level-1 dot/AXPY and selected GEMV, SYMV, GEMM,
...
Every measured candidate score is within or below the noisy two-pass baseline range. A later unmeasured review
fix separates exceptional matrix arithmetic from reordered/FMA/zero-skipping Level-1 paths; the measured fixture
did not enter those removed whole-storage matrix branches. The evidence rules out an obvious structural
regression at the selected call sites but does not establish a speedup or new crossover. Allocation
