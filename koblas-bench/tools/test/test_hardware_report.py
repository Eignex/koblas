...
import hardware_bundle as bundle_processor
...
def write_bundle(root, *, arm="built-in", log="ok", pass_two_rows=None):
    ...
    pass_rows = bundle_processor.load_pass(raw / "pass-1.json", arm, 1, 1)
    pass_rows.extend(bundle_processor.load_pass(raw / "pass-1.json", arm, 2, 1))
    aggregates = bundle_processor.aggregate_rows(pass_rows)
    ...
    bundle_processor.write_rows(root, aggregates)
    (root / "summary.txt").write_text("summary\n")
    bundle_processor.checksums(root)
...
def write_matched_bundle(root):
    ...
    for arm, score in (("built-in", 2.0), ("openblas", 1.0)):
        ...
        for number in (1, 2):
            ...
            all_pass_rows.extend(bundle_processor.load_pass(path, arm, number, 1))
    (root / "metadata.json").write_text(json.dumps({
        "schema_version": 1,
        "mode": "standard",
    ...
    bundle_processor.write_rows(root, bundle_processor.aggregate_rows(all_pass_rows))
    (root / "summary.txt").write_text("summary\n")
    bundle_processor.checksums(root)
...
class HardwareReportTest(unittest.TestCase):
    ...
    def test_case_id_excludes_arm(self):
        built_in = bundle_processor.stable_case_id(jmh_row(), 1)
        openblas = bundle_processor.stable_case_id(jmh_row("openblas"), 1)
    ...
    def test_v1_case_id_normalizes_only_the_historical_triangle_default(self):
        row = jmh_row()
        row["benchmark"] = f"{bundle_processor.V1_SPARSE_PRODUCT_BENCHMARK}.preparedGemv"
        row["params"]["triangleVariant"] = bundle_processor.V1_TRIANGLE_VARIANT

        self.assertEqual(
            bundle_processor.stable_case_id(row, 1),
            f"v1:{bundle_processor.V1_SPARSE_PRODUCT_BENCHMARK}.preparedGemv[n=64]",
        )

        row["params"]["triangleVariant"] = "lower-trans-nonunit"
        with self.assertRaisesRegex(bundle_processor.ReportError, "developer triangular variant"):
            bundle_processor.stable_case_id(row, 1)
    ...
    def test_retained_onemkl_report_preserves_historical_v1_case_ids(self):
        ...
        def built_in_ids(archive):
            with bundle_processor.bundle_directory(archive) as bundle:
                metadata = json.loads((bundle / "metadata.json").read_text())
                raw = bundle_processor.raw_entries(bundle / "raw" / "built-in" / "pass-1.json")
                return {
                    bundle_processor.stable_case_id(row, metadata["workload"]["version"])
                    for row in raw
                }
    ...
    def test_missing_case_fails(self):
        with self.assertRaisesRegex(bundle_processor.ReportError, "expected 2 cases"):
            bundle_processor.check_pass([{"case_id": "one"}], "built-in", 2)

    def test_fork_failure_pattern(self):
        self.assertIsNotNone(bundle_processor.FAILURE_PATTERN.search("EXCEPTION: <ERROR> setup failed"))
    ...
    def test_paired_ratio_requires_matching_case(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            rows.extend(bundle_processor.load_pass(path, "built-in", 1, 1))
            path.write_text(json.dumps([jmh_row("openblas", score=1.0)]))
            rows.extend(bundle_processor.load_pass(path, "openblas", 1, 1))

        aggregates = bundle_processor.aggregate_rows(rows)
    ...
    def test_invalid_schema_fails_validation(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaisesRegex(bundle_processor.ReportError, "unsupported"):
                bundle_processor.validate_directory(bundle)
    ...
    def test_validator_detects_fork_failure_in_raw_log(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaisesRegex(bundle_processor.ReportError, "fork failure"):
                bundle_processor.validate_directory(bundle)
    ...
    def test_validator_detects_raw_case_mismatch(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaisesRegex(bundle_processor.ReportError, "identical case IDs"):
                bundle_processor.validate_directory(bundle)
    ...
    def test_validator_requires_checksum_for_every_file(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaisesRegex(bundle_processor.ReportError, "does not cover"):
                bundle_processor.validate_directory(bundle)
    ...
    def test_validator_recomputes_aggregate_rows_and_ratios(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            bundle_processor.checksums(bundle)

            with self.assertRaisesRegex(bundle_processor.ReportError, "does not match raw measurements"):
                bundle_processor.validate_directory(bundle)
    ...
    def test_validator_rejects_raw_arm_substitution(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaisesRegex(bundle_processor.ReportError, "declared openblas arm"):
                bundle_processor.validate_directory(bundle)
    ...
    def test_validator_rejects_raw_measurement_setting_change(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaisesRegex(bundle_processor.ReportError, "measurementTime"):
                bundle_processor.validate_directory(bundle)
