...
CLONES static void blocks(int n,double a,double*v,int stride){
 (void)stride;if(a==1||n<=0)return;
 if(n>=128){
  while((uintptr_t)v&31){*v++*=a;--n;}
  for(;n>=16;n-=16,v+=16)for(int i=0;i<16;++i)v[i]*=a;
 }
 for(int i=0;i<n;++i)v[i]*=a;
}
int main(void){
 ...
 scale_fn f[]={baseline,pointers,pipeline,blocks,ob_leaf,ob_aligned,dlsym(ob,"cblas_dscal"),dlsym(mk,"cblas_dscal")};
 const char*names[]={"current","pointers","pipeline","blocks","openblas-leaf","openblas-aligned","openblas","onemkl"};
 const char*modes[]={"negate","powers-two","reset"};int lengths[]={64,128,256,4096,65536};
 puts("n,offset_bytes,mode,implementation,sample,operations,cpu_ns,wall_ns,cpu_ns_per_op,wall_ns_per_op");
 for(int shape=0;shape<5;++shape)for(int offset=0;offset<32;offset+=8)for(int mode=0;mode<3;++mode){
  ...
  for(int sample=-2;sample<7;++sample){uint64_t cpu[8]={0},wall[8]={0};
   for(int round=0;round<32;++round)for(int step=0;step<8;++step){int arm=(step+round+sample+8)%8;
