#!/usr/bin/env bash
set -euo pipefail
out=/tmp/scal-openblas-source/suite
mkdir -p "$out"
...
for pass in 1 2 3; do
 ...
 for mode in $order; do
  ...
  args=("--cases=$out/cases.txt" "--output=$out/p$pass-$mode.csv" --samples=5 --warmups=5 --target-ms=200 "--pass=$pass" "--source-commit=$commit" "--dirty=$dirty")
  case $mode in
