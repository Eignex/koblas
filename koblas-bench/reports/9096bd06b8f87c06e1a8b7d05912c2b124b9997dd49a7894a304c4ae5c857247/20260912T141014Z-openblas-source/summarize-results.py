...
print('Thread CPU ns/op; seven samples. Misaligned rows pool offsets 8, 16 and 24 bytes. Aligned rows use offset zero.\n')
print('| n | Timing | Alignment | Alignment only | Fixed blocks | OpenBLAS leaf plus alignment | oneMKL | OpenBLAS |')
print('|---:|---|---|---:|---:|---:|---:|---:|')
leaf = defaultdict(list)
for row in csv.DictReader((root/'blocks.csv').open()):
    alignment = 'aligned' if row['offset_bytes'] == '0' else 'misaligned'
    leaf[int(row['n']),row['mode'],alignment,row['implementation']].append(float(row['cpu_ns_per_op']))
for n in (64,128,256,4096,65536):
    for mode in ('negate','powers-two','reset'):
        for alignment in ('aligned','misaligned'):
            values = [st.median(leaf[n,mode,alignment,a]) for a in ('current','blocks','openblas-aligned','onemkl','openblas')]
            print('| '+str(n)+' | '+mode+' | '+alignment+' | '+' | '.join(f'{x:.1f}' for x in values)+' |')
if (root/'suite').exists():
    ...
    print('| Case | Native original | Native blocks | oneMKL | OpenBLAS |')
    print('|---|---:|---:|---:|---:|')
