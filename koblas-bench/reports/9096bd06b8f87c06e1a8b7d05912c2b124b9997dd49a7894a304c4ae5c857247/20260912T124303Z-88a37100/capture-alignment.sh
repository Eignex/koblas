#!/usr/bin/env bash
set -euo pipefail
out=/tmp/scal-gather-aligned-paired
mkdir -p "$out"
...
for pass in 1 2 3; do
 ...
 for mode in $order; do
  ...
  args=("--cases=$out/cases.txt" "--output=$out/p$pass-$mode.csv" --samples=5 --warmups=3 --target-ms=50 "--pass=$pass" "--source-commit=$commit" --dirty=true)
  case $mode in
