...
# CHOLMOD wants LAPACK where UMFPACK wanted only BLAS, and the bundled OpenBLAS carries both, so both
# variables point at it. Naming LAPACK_LIBRARIES also settles where it comes from: SuiteSparseLAPACK takes a
# supplied one as-is and returns, where falling through to cmake's own FindLAPACK searches the host and
# takes whatever it finds, which on a machine with no system LAPACK is nothing.
#
# CUDA is opt-out upstream, so a build host that happens to have it would otherwise put a CUDA runtime
# dependency into CHOLMOD. Static archives are never copied into the bundle, so building them would only
# cost compile time.
#
