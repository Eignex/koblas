...
/** Checks the native CHOLMOD sparse product bindings against the reference implementation. */
class CholmodSparseBlasNativeConformanceTest {
    ...
    fun `the binding reports what it is`() {
        assertEquals(BackendNames.CHOLMOD, cholmod.name)
        assertEquals(HOST_BACKEND_PRIORITY, cholmod.priority, "every koblas host binding registers at one priority")
    }

    /** The sparse matrix half alone: the factorization half is where KLU, UMFPACK and BASICLU register. */
    @Test
    fun `discovery registers it as the sparse matrix half`() {
        // Discovery is explicit, so a run holding this class alone would otherwise read the fallbacks.
        discoverBackends()

        assertEquals(BackendNames.CHOLMOD, koblas.sparseBlas.name, "discovery should have registered the backend")
    }
    ...
    /** A zero beta overwrites the destination without reading it, so it may arrive holding anything. */
    @Test
    fun `gemm overwrites the destination when beta is zero`() {
        val rng = Random(20260936)
        val a = sparse(9, 6, rng)
        val b = randomMatrix(6, 3, rng)
        val c = randomMatrix(9, 3, rng)

        val expected = F64DenseMatrix.wrap(9, 3, c.data.copyOf())
        F64ReferenceSparseLinearAlgebra.gemm(1.0, a, false, b, false, 0.0, expected)
        val actual = F64DenseMatrix.wrap(9, 3, c.data.copyOf())
        cholmod.gemm(1.0, a, false, b, false, 0.0, actual)

        assertClose(expected, actual, "beta zero", tolerance = 1e-12)
    }

    /**
     * Portable by decision rather than by omission: a single right-hand side cannot amortise the call. The
     * answers still have to agree.
     */
    @Test
