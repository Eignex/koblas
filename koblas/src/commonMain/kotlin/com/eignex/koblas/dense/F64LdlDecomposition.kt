...
public class F64LdlDecomposition @UnsafeKoblasApi constructor(
    ...
    public val pivotBlocks: List<F64LdlPivotBlock>
        get() {
            val blocks = ArrayList<F64LdlPivotBlock>()
            var position = 0
            while (position < n) {
                val raw = rawPivotBuffer[position]
                if (raw > 0) {
                    blocks += F64LdlPivotBlock.OneByOne(position, raw - 1)
                    position++
                } else {
                    blocks += F64LdlPivotBlock.TwoByTwo(position, -raw - 1)
                    position += 2
                }
            }
            return blocks
...
/** A 0-based Bunch-Kaufman pivot block in an [F64LdlDecomposition]. */
public sealed interface F64LdlPivotBlock {
    ...
    public data class OneByOne(override val position: Int, override val interchangedWith: Int) : F64LdlPivotBlock {
        override val interchangePosition: Int get() = position
    }

    /** A 2×2 pivot covering [position] and `position + 1`. */
    public data class TwoByTwo(override val position: Int, override val interchangedWith: Int) : F64LdlPivotBlock {
