...
public abstract class BlasAdapter internal constructor(
    ...
    override fun route(query: RouteQuery): BackendRoute? = when (query) {
        is RouteQuery.DenseGemv, is RouteQuery.DenseGemm -> nativeRoute(
            query,
            this,
    ...
    private fun scaleInPlace(view: StridedMatrixView, beta: Double) {
        if (beta == 1.0) return
        if (beta == 0.0) {
