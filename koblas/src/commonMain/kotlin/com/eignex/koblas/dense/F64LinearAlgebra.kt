...
/** QR with column pivoting, `A·P = Q·R`, with the active backend; see [F64Lapack.qrPivoted]. */
...
/** `A⁻¹` from this factorization (LAPACK `dgetri`); see [F64Lapack.invert]. */
public fun F64LuDecomposition.invert(workspace: Workspace? = null): F64DenseMatrix = koblas.invert(this, workspace)

/**
 * Reciprocal condition estimate, given the 1-norm [anorm] of the matrix it came from; see [F64Lapack.rcond].
...
/** Solve `A · x = b` for this symmetric indefinite factorization; see [F64Lapack.solve]. */
...
/** Least-squares solution of `A · x ≈ b` for an overdetermined system; see [F64Lapack.solveLeastSquares]. */
...
/** Minimum-norm solution of an underdetermined `A · x = b`; see [F64Lapack.solveMinimumNorm]. */
...
/** `Q · y`, or `Qᵀ · y` when [transpose], without forming `Q`; see [F64Lapack.applyQ]. */
...
/** Least-squares solution against a rank-revealing factorization; see [F64Lapack.solveLeastSquares]. */
