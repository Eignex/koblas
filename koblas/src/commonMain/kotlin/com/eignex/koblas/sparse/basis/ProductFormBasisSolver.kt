...
public class ProductFormBasisSolver(
    ...
    private var base: SparseFactorization? = null
     ...
     * and complained about, so [RefactorizeReason.FACTOR_ASKED] is not a cause it can report.
     */
    private var advisedReason: RefactorizeReason? = null

    override val refactorizeReason: RefactorizeReason? get() = advisedReason
    ...
    override fun ftran(x: IndexedVector, expectedDensity: Double) {
        ...
        if (expectedDensity < REACHABLE_FTRAN_MAX_DENSITY && factors is SparseMarkowitzLu) {
            if (denseDirty) {
                dense.fill(0.0)
                denseDirty = false
            }
    ...
    override fun update(pivotRow: Int, entering: Int, spike: IndexedVector, pivotEta: IndexedVector?): BasisUpdate {
        ...
        advisedReason = if (updateCount >= etaLimit) RefactorizeReason.UPDATES_WORN else null
    ...
    private fun solvable(x: IndexedVector): SparseFactorization {
