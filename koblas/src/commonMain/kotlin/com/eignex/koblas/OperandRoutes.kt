...
public fun KoblasEngine.routeOf(
    ...
    return productRoute(alpha, a, transposeA, b, transposeB, beta, c.rows, c.cols, c.values.size,
...
public fun KoblasEngine.routeOf(
    ...
    return productRoute(alpha, a, transposeA, b, transposeB, 0.0, rows, columns,
...
private fun KoblasEngine.productRoute(
    ...
    if (alpha == 0.0 && elements != null) {
        ...
        return DenseMatrixRoute(DenseMatrixOperation.Gemm, RouteKind.NoWork, DENSE_SCHEDULING, "gemm",
            scaling, 0, "the generic product only scales its destination")
    }
    ...
    return when {
        left != null && right != null -> {
            val operation = if (elements == null) SparseMatrixOperation.GemmSparse else SparseMatrixOperation.GemmSparseDense
            val route = engine.routeOf(operation, SparseCall(left, alpha, beta,
                destinationElements = elements, depth = depth, transposeSparse = transposeA))
            ...
            SparseMatrixRoute(route.operation, if (empty) RouteKind.NoWork else route.kind, route.scheduling,
        ...
        left != null -> engine.routeOf(SparseMatrixOperation.GemmDense, SparseCall(
            left, alpha, beta, destinationElements = elements, depth = depth, rightHandSides = columns,
            transposeSparse = transposeA, transposeDense = transposeB,
        ))

        right != null -> engine.routeOf(SparseMatrixOperation.GemmDenseRight, SparseCall(
        ...
        else -> routeOf(DenseMatrixOperation.Gemm, DenseCall(rows, columns, alpha, beta, depth = depth,
            transposeA = transposeA, transposeB = transposeB, aliased = aliased))
    }
}
