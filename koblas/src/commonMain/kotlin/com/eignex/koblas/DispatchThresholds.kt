 ...
 * it follows the ratio of work to data. Level 1 is a run length rather than a dimension, and is overridable
 * at runtime rather than a compile-time constant, so retuning it for a machine with a different vector width
 * does not take a recompile. Level 2 does `O(n²)` work over `O(n²)` data, so there is nothing
