...
- Free-function arithmetic over the views: [dot], [axpy], [scale], [norm2], [asum], [iamax], [copy],
  [swap], [addOuter], [matVec], [transpose], [forEachStored]; the triangular solves [trsv] / [trsm];
...
- The swappable [LinearAlgebra] backend for the heavier dense ops (`gemv` / `gemm` in full BLAS
  ...
  with [determinant]) with the portable [ReferenceLinearAlgebra], the [platformLinearAlgebra]
  native-backend seam, the resolved default [koblas], the [LuDecomposition] result, and the ergonomic
  [lu] / [LuDecomposition.solve][solve] / [matMul] entry points.
