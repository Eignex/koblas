...
class BoundsAndShapeTest {
    ...
    private val dense = F64DenseMatrix.of(arrayOf(doubleArrayOf(1.0, 2.0, 3.0), doubleArrayOf(4.0, 5.0, 6.0)))
    private val sparse = F64SparseMatrix.ofColumns(2, 3, listOf(listOf(0 to 1.0), listOf(1 to 5.0), listOf()))

    @Test
    fun `every matrix storage rejects an index outside the shape`() {
        for (a in listOf<F64MatrixLike>(dense, sparse)) {
    ...
    fun `every vector storage rejects an index outside the size`() {
        val vectors = listOf<F64VectorLike>(
            F64DenseVector.of(doubleArrayOf(1.0, 2.0)),
            F64SparseVector.of(2, intArrayOf(0), doubleArrayOf(1.0)),
        )
        for (v in vectors) {
    ...
    fun `a shape whose entry count overflows an Int is rejected`() {
        assertFailsWith<DimensionMismatch>("wraps to exactly zero") { F64DenseMatrix.zero(65536, 65536) }
        assertFailsWith<DimensionMismatch>("wraps negative") { F64DenseMatrix.zero(46341, 46341) }
        assertFailsWith<DimensionMismatch>("one huge dimension") { F64DenseMatrix.zero(2, 1073741824) }
        assertEquals(4, F64DenseMatrix.zero(2, 2).data.size, "an ordinary shape still works")
    }

    @Test
    fun `a negative shape is rejected as a shape error`() {
        assertFailsWith<DimensionMismatch> { F64DenseMatrix.zero(-1, 2) }
        assertFailsWith<DimensionMismatch> { F64DenseMatrix.zero(2, -1) }
        assertFailsWith<DimensionMismatch> { F64DenseVector.zero(-3) }
        assertFailsWith<DimensionMismatch> { F64SparseVector.wrap(-5, IntArray(0), DoubleArray(0)) }
        assertFailsWith<DimensionMismatch> { F64SparseVector.of(-3, IntArray(0), DoubleArray(0)) }
        assertFailsWith<DimensionMismatch> {
            F64SparseMatrix.ofTriplets(
                -1,
                1,
                IntArray(0),
                IntArray(0),
    ...
    fun `a negative size cannot arrive through deserialization either`() {
        assertFailsWith<DimensionMismatch> {
            Json.decodeFromString<F64VectorStorage>(
                """{"type":"F64SparseVector","size":-5,"indices":[],"values":[]}""",
            )
        }
    }
}
