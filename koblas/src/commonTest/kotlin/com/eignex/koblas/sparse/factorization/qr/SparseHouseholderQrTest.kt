...
class SparseHouseholderQrTest {
    ...
    fun `the least squares solution satisfies the normal equations`() {
        ...
        for ((m, n) in listOf(1 to 1, 5 to 3, 40 to 12, 97 to 61)) {
            ...
            val x = SparseHouseholderQr.factor(a).solve(b)
    ...
    fun `the residual is orthogonal to every column of the matrix`() {
        ...
        val residual = multiply(a, SparseHouseholderQr.factor(a).solve(b)).mapIndexed { i, v -> v - b[i] }
    ...
    fun `a square system is solved exactly`() {
        ...
        val x = SparseHouseholderQr.factor(a).solve(multiply(a, expected))
    ...
    fun `R transposed times R reproduces A transposed times A`() {
        ...
        val r = SparseHouseholderQr.factor(a).r
    ...
    fun `applying Q transposed and then Q returns the original vector`() {
        ...
        val qr = SparseHouseholderQr.factor(a)
    ...
    fun `a wider than tall matrix is rejected`() {
        ...
        assertFailsWith<IllegalArgumentException> { SparseHouseholderQr.factor(a) }
    }
    ...
    fun `large finite values do not overflow the reflector`() {
        ...
        val x = SparseHouseholderQr.factor(a).solve(doubleArrayOf(1e200, 1e200))
    ...
    fun `small finite values do not lose the reflector tail`() {
        ...
        val x = SparseHouseholderQr.factor(a).solve(doubleArrayOf(0.0, 1e-200))
