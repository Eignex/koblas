...
class EtaBasisTest {
    ...
    fun `solves after a single rank-1 basis update`() {
        ...
        val n = 4
        val eta = EtaBasis.of(identity(n).lu())

        val p = 2
        val q = doubleArrayOf(0.5, -1.0, 3.0, 2.0) // new column entering slot p
        val spike = eta.solve(q) // spike = B⁻¹ q, computed before the update (B is still identity)
        eta.update(p, spike)
        ...
        assertClose(x, eta.solve(b), "single update", tolerance = 1e-9)
    }
    ...
    fun `solves and transpose-solves after multiple rank-1 updates`() {
        ...
        assertClose(x, eta.solve(b), "ftran after two updates", tolerance = 1e-9)

        val bt = DoubleArray(n) { j -> (0 until n).sumOf { i -> basis[i][j] * x[i] } } // Bᵀ x
        assertClose(x, eta.solve(bt, transpose = true), "btran after two updates", tolerance = 1e-9)
    }
}
