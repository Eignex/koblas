...
class SparseFactorizationTest {
    private val reference = ReferenceSparseLinearAlgebra

    /** The fallback any backend can be given, for one that cannot update its own factors. */
    private fun refactoring(basis: SparseMatrix): BasisFactorization =
        RefactoringBasisFactorization(reference, basis, reference.factor(basis))
    ...
    fun `the refactoring fallback rejects a rectangular basis`() {
        val wide = SparseMatrix.ofColumns(1, 2, listOf(listOf(0 to 1.0), listOf(0 to 2.0)))
    ...
    fun `the refactoring fallback follows a column replacement`() {
        ...
        val n = 9
        val basis = sparseConformanceSystem(n, rng)
        val entering = SparseVector.of(n, intArrayOf(0, 4, 7), doubleArrayOf(3.0, -2.0, 11.0))
        val b = DoubleArray(n) { rng.nextDouble(-1.0, 1.0) }
    ...
    fun `a replacement that makes the basis singular is reported rather than thrown`() {
        ...
        val duplicate = SparseVector.of(2, intArrayOf(0), doubleArrayOf(1.0))
