...
class SparseLuAnalysisTest {
    private class RepeatedProvider : RepeatedSparseLu {
...
internal fun assertSymbolicAnalysisReuses(provider: RepeatedSparseLu) {
    ...
    provider.analyze(first).use { analysis ->
        ...
        factor = analysis.refactor(factor, changed)
        factor.use {
            val expected = ReferenceSparseLinearAlgebra.factor(changed).solve(doubleArrayOf(8.0, 12.0))
