...
class ProductFormBasisSolverTest {
    ...
    fun `an update past the eta limit asks for a rebuild`() {
        ...
        val n = 6
        val a = simplexMatrix(n, rng)
        val solver = ProductFormBasisSolver(a, reference, etaLimit = 2)
    ...
    fun `a rebuild that throws leaves the basis the solver already had`() {
        ...
        val solver = ProductFormBasisSolver(a, lu)
        solver.refactorize(logicalBasis(n))
    ...
    fun `rebuild and close release each owned base factor once`() {
        ...
        val solver = ProductFormBasisSolver(simplexMatrix(n, Random(20261011)), tracking)
    ...
    private class TrackingFactor(private val delegate: SparseLuFactorization) :
        SparseLuFactorization by delegate {
        var closes = 0
            private set
    ...
    fun `the reachable ftran agrees with the dense sweep across densities`() {
        ...
        val solver = ProductFormBasisSolver(a, ReferenceSparseLinearAlgebra)
