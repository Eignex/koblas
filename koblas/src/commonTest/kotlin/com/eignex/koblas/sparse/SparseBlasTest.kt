...
class SparseBlasTest {
    ...
    fun `symmetric products read only the selected triangle`() {
        ...
        for (lower in booleanArrayOf(false, true)) {
            ...
            val y = DoubleArray(3)
            koblas.symv(1.0, a, x, 0.0, y, lower)
            ...
            koblas.symm(1.0, a, b, 0.0, left, lower)
            assertContentEquals(expectedVector, left.values.copyOfRange(0, 3))
            val rightInput = DenseMatrix.wrap(2, 3, doubleArrayOf(2.0, 11.0, -3.0, 13.0, 5.0, 17.0))
            val right = DenseMatrix.zero(2, 3)
            koblas.symm(1.0, a, rightInput, 0.0, right, lower, right = true)
    ...
    fun `symmetric products stage aliases and honor scalar no reads`() {
        ...
        val shared = a.values
        koblas.symv(1.0, a, shared, 0.0, shared, lower = true)
        ...
        koblas.symv(0.0, poisoned, doubleArrayOf(Double.NaN), 0.0, y)
    ...
    fun `sparse result gemm supports scaling transpose and structure`() {
        ...
        val actual = koblas.gemm(-2.0, a, true, b, false)
    ...
    fun `sparse syrk provides dense and selected sparse results`() {
        ...
        val exceptional = SparseMatrix.ofColumns(2, 1, listOf(listOf(0 to Double.POSITIVE_INFINITY, 1 to 0.0)))
        assertTrue(koblas.syrk(exceptional)[1, 0].isNaN())
        val implicit = SparseMatrix.ofColumns(2, 1, listOf(listOf(0 to Double.POSITIVE_INFINITY)))
        val implicitDense = DenseMatrix.zero(2)
        koblas.syrk(1.0, implicit, false, 0.0, implicitDense)
    ...
    fun `sparse syrk follows diagonal stored adjacency in both orientations`() {
        ...
        for (transpose in booleanArrayOf(false, true)) {
            val result = koblas.syrk(diagonal, transpose)
    ...
    fun `sparse syrk handles hypersparse rectangular adjacency`() {
        ...
        for (lower in booleanArrayOf(false, true)) {
            val ordinary = koblas.syrk(a, transpose = false, lower)
            ...
            val transposed = koblas.syrk(a, transpose = true, lower)
    ...
    fun `empty products retain requested shapes and scalar behavior`() {
        ...
        val sparse = koblas.gemm(2.0, a, false, b, false)
        ...
        koblas.gemm(0.0, a, false, b, false, -0.5, dense)
        assertTrue(dense.values.all { it == -2.0 })

        val emptySymmetric = SparseMatrix.ofColumns(0, 0, emptyList())
        koblas.symv(1.0, emptySymmetric, DoubleArray(0), 0.0, DoubleArray(0))
        koblas.symm(1.0, emptySymmetric, DenseMatrix.zero(0, 3), 0.0, DenseMatrix.zero(0, 3))
    }
    ...
    fun `scaled addition retains union and subtraction direction`() {
        ...
        val zero = koblas.addScaled(-0.0, a, false, b)
    ...
    @Test
    fun `symmetric and rank products agree with the independent sparse oracle`() {
        val a = SparseMatrix.ofColumns(
            4,
            4,
            listOf(
                listOf(0 to 2.0, 2 to -1.5),
                listOf(1 to 3.25, 3 to 0.5),
                listOf(2 to 4.0),
                listOf(3 to -2.25),
            ),
        )
        val x = doubleArrayOf(1.5, -2.0, 0.25, 3.0)
        for (lower in booleanArrayOf(false, true)) {
            val expected = DoubleArray(4) { 0.5 * it }
            val actual = expected.copyOf()
            ReferenceSparseBlas.symv(0.75, a, x, -1.5, expected, lower)
            koblas.symv(0.75, a, x, -1.5, actual, lower)
            assertContentEquals(expected, actual, "symv lower=$lower")

            val expectedRank = DenseMatrix.wrap(4, 4, DoubleArray(16) { 0.25 * it })
            val actualRank = DenseMatrix.wrap(4, 4, expectedRank.values.copyOf())
            ReferenceSparseBlas.syrk(1.25, a, false, -0.5, expectedRank, lower)
            koblas.syrk(1.25, a, false, -0.5, actualRank, lower)
            assertContentEquals(expectedRank.values, actualRank.values, "syrk lower=$lower")
        }
        assertEquals(ReferenceSparseBlas.syrk(a), koblas.syrk(a), "fresh sparse syrk")
        assertEquals(ReferenceSparseBlas.transpose(a), koblas.transpose(a), "transpose")
    }
}
