...
internal fun assertSyrAgreesWithReference(blas: Blas) {
    val rng = Random(20260831)
    for (n in intArrayOf(1, 7, 31)) {
        for (lower in booleanArrayOf(true, false)) {
            val x = DenseVector.of(DoubleArray(n) { rng.nextDouble(-1.0, 1.0) })
            val y = DenseVector.of(DoubleArray(n) { rng.nextDouble(-1.0, 1.0) })
            val seed = randomMatrix(n, n, rng)
            val syrExpected = DenseMatrix(n, n, seed.data.copyOf())
            val syrActual = DenseMatrix(n, n, seed.data.copyOf())
            ...
            val syr2Expected = DenseMatrix(n, n, seed.data.copyOf())
            val syr2Actual = DenseMatrix(n, n, seed.data.copyOf())
...
internal fun assertSyr2kAgreesWithReference(blas: Blas, sizes: IntArray) {
...
private fun compareTriangle(expected: DenseMatrix, actual: DenseMatrix, n: Int, lower: Boolean, context: String) {
    for (i in 0 until n) {
        for (j in 0 until n) {
            val ctx = "$context ($i;$j)"
...
internal fun assertDegenerateShapesFollowBlasQuickReturns(blas: Blas) {
...
internal fun assertStridedProductsAgreeWithReference(blas: Blas) {
    ...
    val x = StridedVectorView(xData, 0, 2, 2)
    val expectedVector = StridedVectorView(expectedY, 0, 3, 2)
    val actualVector = StridedVectorView(actualY, 0, 3, 2)
