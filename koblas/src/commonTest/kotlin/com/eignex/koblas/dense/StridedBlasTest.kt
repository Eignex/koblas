...
class StridedBlasTest {
    ...
    fun `norm2 over a strided view propagates a NaN entry`() {
        ...
        val allNaN = StridedVectorView(doubleArrayOf(Double.NaN), 0, 1)
        val zerosAndNaN = StridedVectorView(doubleArrayOf(0.0, Double.NaN, 0.0), 0, 3)
        ...
        assertEquals(0.0, StridedVectorView(doubleArrayOf(0.0, 0.0), 0, 2).norm2(), "an all-zero view")
    }
    ...
    fun `strided gemv forms zero products from nonzero alpha`() {
        ...
        val a = DenseMatrix(2, 2, doubleArrayOf(Double.POSITIVE_INFINITY, 1.0, 2.0, 3.0)).asView()
        val x = StridedVectorView(doubleArrayOf(0.0, 1.0), 0, 2)
    ...
    fun `strided gemm scales without forming products when alpha is zero`() {
        val a = DenseMatrix(1, 1, doubleArrayOf(Double.POSITIVE_INFINITY)).asView()
        val b = DenseMatrix(1, 1, doubleArrayOf(1.0)).asView()
    ...
    fun `gemv reads a panel and strided vectors without copies`() {
        ...
        val x = StridedVectorView(xBuffer, 0, 2, 2)
        val y = StridedVectorView(yBuffer, 0, 3, 2)

        ReferenceBlas.gemv(2.0, a, x, 0.5, y)
    ...
    fun `strided gemv leaves its destination alone for an empty matrix`() {
        ...
        ReferenceBlas.gemv(
            ...
            0.0,
            StridedVectorView(yBuffer, 2, 3, -1),
            transpose = true,
        )
    ...
    fun `view BLAS rejects an overlapping destination`() {
        val owner = DenseMatrix.diagonal(3)
        val matrix = owner.asView()
        val x = matrix.column(0)
        val y = matrix.column(1)

        assertFailsWith<IllegalArgumentException> {
            ReferenceBlas.gemv(1.0, matrix, x, 0.0, y)
        }
        assertFailsWith<IllegalArgumentException> {
            ReferenceBlas.gemm(1.0, matrix, false, matrix, false, 0.0, matrix)
        }
    }
    ...
    fun `vector operations mutate borrowed rows and slices`() {
        ...
        val increment = StridedVectorView(doubleArrayOf(1.0, 9.0, 1.0, 9.0, 1.0), 0, 3, 2)
