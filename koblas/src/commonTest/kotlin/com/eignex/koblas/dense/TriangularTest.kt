...
class TriangularTest {
    ...
    /**
     * `dtrsv` divides by the diagonal without testing it, so a singular triangle answers with infinities
     * rather than an exception. The sparse half reports it instead, because it has to look the diagonal up
     * to divide by it and can tell a stored zero from a missing entry while it is there. Both are pinned so
