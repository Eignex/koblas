...
class LinearAlgebraTest {
    ...
    fun `routed LU factor agrees with the reference for rectangular matrices`() {
        ...
        for ((rows, cols) in listOf(5 to 3, 3 to 5)) {
            ...
            assertEquals(expected.rowOrder().toList(), actual.rowOrder().toList(), "pivots ${rows}x$cols")
        }
    }
    ...
    fun `LU exposes a zero based defensive row permutation`() {
        ...
        assertContentEquals(intArrayOf(1, 0), factor.rowOrder())
        assertEquals(1, factor.rowAt(0))
        val copy = factor.rowOrder()
        copy[0] = 0
