...
class BranchCoverageTest {
    ...
    fun `matVec agrees across every storage combination`() {
        ...
        // A MatrixLike koblas does not know, so matVec takes its entry-by-entry path.
        ...
        for ((name, matrix) in listOf<Pair<String, MatrixLike>>(
            "dense" to dense,
            "sparse" to sparse,
            "foreign" to foreign,
        )) {
    ...
    fun `rcond spans well and ill conditioned matrices`() {
        val identity = DenseMatrix.diagonal(6)
        assertTrue(
