...
public class IndexedVector(public val size: Int) {
    init {
        requireHfactorShape(size >= 0) { "negative size: $size" }
    }
    ...
    public fun scatterColumn(a: SparseMatrix, j: Int) {
        requireHfactorShape(a.rows == size) { "scatterColumn: rows ${a.rows} != $size" }
        requireHfactorIndex(j in 0 until a.cols) { "index $j outside [0,${a.cols})" }
        clear()
    ...
    public fun scatter(dense: DoubleArray) {
        requireHfactorShape(dense.size == size) { "scatter: dense size ${dense.size} != $size" }
        clear()
    ...
    public fun gather(out: DoubleArray): DoubleArray {
        requireHfactorShape(out.size == size) { "gather: out size ${out.size} != $size" }
        out.fill(0.0)
